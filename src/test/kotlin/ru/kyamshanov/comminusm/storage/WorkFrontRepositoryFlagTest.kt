...
class WorkFrontRepositoryFlagTest {
    ...
    fun `findAllActivated returns all work fronts`() {
        ...
        repo.upsert(DomainToModelAdapter.toDomain(WorkFront(uuid1, "world", 10, 64, 10)))
        repo.upsert(DomainToModelAdapter.toDomain(WorkFront(uuid2, "world", 20, 64, 20)))
    ...
    fun `findAllActivated returns updated front after upsert`() {
        ...
        repo.upsert(DomainToModelAdapter.toDomain(WorkFront(uuid, "world", 10, 64, 10)))
        repo.upsert(DomainToModelAdapter.toDomain(WorkFront(uuid, "world", 50, 64, 50)))
    ...
    fun `findAllActivated returns fronts across multiple worlds`() {
        val uuid1 = UUID.randomUUID()
        val uuid2 = UUID.randomUUID()
        repo.upsert(DomainToModelAdapter.toDomain(WorkFront(uuid1, "world", 0, 64, 0)))
        repo.upsert(DomainToModelAdapter.toDomain(WorkFront(uuid2, "nether", 100, 64, 100)))
