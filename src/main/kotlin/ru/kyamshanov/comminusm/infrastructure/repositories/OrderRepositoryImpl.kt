...
class OrderRepositoryImpl(
    ...
    override fun findByOwner(uuid: UUID): Order? {
        val stmt =
            conn.prepareStatement(
                "SELECT id, owner_uuid, name, level, center_world, center_x, center_y, center_z, radius FROM orders WHERE owner_uuid = ?",
            )
    ...
    override fun findAllInWorld(world: String): List<Order> {
        val stmt =
            conn.prepareStatement(
                "SELECT id, owner_uuid, name, level, center_world, center_x, center_y, center_z, radius FROM orders WHERE center_world = ?",
            )
        stmt.setString(1, world)
        val rs = stmt.executeQuery()
    ...
    override fun findAllActivated(): List<Order> {
        val stmt =
            conn.prepareStatement(
                "SELECT id, owner_uuid, name, level, center_world, center_x, center_y, center_z, radius FROM orders WHERE center_world IS NOT NULL",
            )
        val rs = stmt.executeQuery()
        val result = mutableListOf<Order>()
        while (rs.next()) {
