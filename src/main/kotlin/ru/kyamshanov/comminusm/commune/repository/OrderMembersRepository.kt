...
class OrderMembersRepository(
     ...
     */
    fun removeMember(orderId: Long, playerUuid: UUID): Boolean {
    ...
    fun getMembersOfOrder(orderId: Long): Set<OrderMember> {
        return cache[orderId]?.toSet() ?: emptySet()
    }
     ...
     */
    fun isMember(orderId: Long, playerUuid: UUID): Boolean {
        return cache[orderId]?.any { it.playerUuid == playerUuid } ?: false
    }

    /**
     * Get members of a specific type ("native" or "commune").
     */
    fun getMembersWithType(orderId: Long, grantedVia: String): Set<OrderMember> {
        return cache[orderId]?.filter { it.grantedVia == grantedVia }?.toSet() ?: emptySet()
    }
}
