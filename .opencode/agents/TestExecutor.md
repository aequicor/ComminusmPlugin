...
4. INTEGRATION — if the project defines an integration test target, run it.
           ...
           a documented integration target in `.opencode/_shared.md`. If none → skip; note this in report.
...
If during the run you observe non-critical issues that are **outside this run's scope** (a flaky test in another module that didn't fail today, a deprecated runner option, a slow test that could be sharded) — record one entry via `.opencode/skills/tech-debt-record/SKILL.md`. Cap: max 3 per run. Real failures and broken tests are not tech-debt — they are reported in the result block.
