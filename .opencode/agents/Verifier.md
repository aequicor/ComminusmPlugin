...
---
# Role: Verifier
Mode-driven ревью (build/review/DoD/trace/mutation-sample)
...
1. Run the project's test command (`./gradlew test`; per-module if monorepo).
...
3. **Anti-pattern (always):**
   ...
   - "How to verify" includes commands the project doesn't expose → **MEDIUM** "Pass E: verify steps reference commands not in this project's stack". Use `./gradlew build` / `./gradlew test` as ground truth.
